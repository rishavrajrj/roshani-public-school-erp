...
export function ParentDocumentView({ reportCard, certificates, childName }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-xl border border-slate-200 shadow-sm print:hidden">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Academic Documents — {childName || 'Ward'}</h2>
          <p className="text-xs text-slate-500 mt-1">Official progress report card and issued certificates</p>
        </div>
          ...
          {!reportCard || reportCard.status !== 'published' ? (
            <div className="bg-white rounded-xl border border-slate-200 p-12 text-center max-w-2xl mx-auto my-8 shadow-sm space-y-4">
              ...
              <p className="text-sm text-slate-500 max-w-md mx-auto">
                The progress report card for {childName || 'your ward'} is undergoing administrative processing or publication. Please check back later.
              </p>
            </div>
          ) : (
        ...
        <div className="space-y-6">
          {certificates.length === 0 ? (
            <div className="bg-white rounded-xl border border-slate-200 p-12 text-center max-w-2xl mx-auto my-8 shadow-sm space-y-4">
              ...
              <p className="text-sm text-slate-500 max-w-md mx-auto">
                No official academic certificates have been issued for {childName || 'your ward'}.
              </p>
            </div>
          ) : selectedCert ? (
            <div className="space-y-4">
