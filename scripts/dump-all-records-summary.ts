  ...
  const results = await Promise.all(
    tables.map(async (t) => {
      try {
        ...
        if (error) return { table: t, error: error.message, count: 0, sample: null }
        return { table: t, count: count ?? 0, sample: data?.[0] || null }
      } catch (err: any) {
        return { table: t, error: err.message, count: 0, sample: null }
      }
    })
  )

  const output = {
    summary: results.map(r => ({ table: r.table, count: r.count })),
    populatedTables: results.filter(r => r.count > 0).map(r => ({
      table: r.table,
      recordCount: r.count,
      sampleColumns: r.sample ? Object.keys(r.sample) : []
    }))
  }

  fs.writeFileSync(path.resolve(process.cwd(), 'scripts', 'db-records-overview.json'), JSON.stringify(output, null, 2))
  console.log('Successfully wrote records overview.')
}
